...
class Interactive:
    ...
    def if_play_songs_first(self):
        ...
        """
        b1 = widgets.Button(description="Yes",
                            layout=widgets.Layout(width="30%"))
        b2 = widgets.Button(description="No",
                            layout=widgets.Layout(width="30%"))
        display(
            HTML(
                "<h2>Before analysing the genre, do you want to play your"
                " audio file?<h2>"
            )
        )
        output = widgets.Output()
        ...
        def yes_play_audio(b: widgets.Button):
            ...
            b3 = widgets.Button(description="End",
                                layout=widgets.Layout(width="30%"))
            output = widgets.Output()
            ...
            display(
                HTML(
                    "<h2>Gotcha! Gonna play the audio for you! <h2>"
                )
            )
            display(ipd.Audio(data=self.y_audio, rate=self.sample_rate))
            display(b3)
        ...
        def no_dont_play(b: widgets.Button):
            ...
            Returns:
                 self.call_out_genre_prediction: The call out
                 genre prediction method
            """
            output = widgets.Output()
            output.clear_output(wait=True)
            clear_output(wait=True)
            display(
                HTML("<h2>Ok, analysing the genre "
                     "for you <h2>"
                     )
            )
        ...
        def end_play_continue(b: widgets.Button):
            ...
            After the users chooses to play the song, create and display
            an end button for users to choose if they want to end playing
            the song and continue on analysing.

            Returns:
                self.call_out_genre_prediction: The call out genre
                prediction method
            """
            output = widgets.Output()
    ...
    def call_out_genre_prediction(self, output: widgets.Output) -> None:
        ...
        """
        output.clear_output(wait=False)
        b4 = widgets.Button(description="Yes",
                            layout=widgets.Layout(width="30%"))
        b5 = widgets.Button(description="No",
                            layout=widgets.Layout(width="30%"))
        ...
        display(
            HTML(
                "<h2> Do you want to know the most popular song in that "
                "genre and similar genres?<h2>"
            )
        )
        output = widgets.Output()
        ...
        def yes_similiar_songs(b: widgets.Button):
            ...
            (
                self.most_popular_song_ids,
                self.most_popular_song,
                self.most_popular_artist,
                self.most_popular_album,
            ) = return_most_popular_song(
                genre=genre, genre_df=total_genre_df, track_df=track_df
            )
            display(
                HTML(
                    "<h3>The most popular songs in the genre {} are {} by {} "
                    "from the albums {} <h3>".format(
                        genre,
                        ", ".join(self.most_popular_song),
                        ", ".join(self.most_popular_artist),
                        ", ".join(self.most_popular_album),
                    )
                )
            )
            self.most_similar_genres = return_similar_genres(
                genre=genre, genre_df=total_genre_df, track_df=track_df, k=10
            )
            display(
                HTML(
                    "<h3>The most similar genres to {} are : {} <h3>".format(
                        genre, ", ".join(self.most_similar_genres)
                    )
                )
            )
            ...
            display(
                HTML(
                    "<h3> Would you like to hear the most popular song "
                    "{} <h3><br>".format(self.most_popular_song[0])
                )
            )

            b6 = widgets.Button(description="Yes",
                                layout=widgets.Layout(width="30%"))
            b7 = widgets.Button(description="No",
                                layout=widgets.Layout(width="30%"))
        ...
        def yes_play_song(b: widgets.Button):
            b8 = widgets.Button(description="End",
                                layout=widgets.Layout(width="30%"))
            output = widgets.Output()
            ...
            display(
                HTML(
                    "<h2>Gotcha! Gonna playing the most popular "
                    "song for you! <h2>"
                )
            )
            ...
            file_path = file_path_df[
                file_path_df["file_path"].str.contains(song_id)]
            ...
            if len(file_path) != 0:
                ...
                display(HTML("<h2>Sorry, the audio could not be "
                             "found! <h2>".format()))
        ...
        def no_dont_play_song(b: widgets.Button):
            ...
            display(
                HTML(
                    "<h2>Ok, would you like to hear a random song from"
        ...
        def yes_play_random_song_from_genre(b: widgets.Button):
            ...
            display(
                HTML(
                    "<h2>Which genre would you like to hear a "
            ...
            def play_random_song_from_selected_genre(change):
                ...
                if random_song is not None:
                    display(
                        HTML(
                            "<h2> Playing song {} by {} from {} .<h2>".format(
                    ...
                    display(
                        HTML("<h2> Sorry, could not find a song from this "
                             "genre.<h2>")
                    )

                b9 = widgets.Button(
                    description="Play another random song?",
                    layout=widgets.Layout(width="30%"),
                )
                b10 = widgets.Button(
                    description="No.", layout=widgets.Layout(width="30%")
                )
        ...
        def no_dont_play_random_song_from_genre(b: widgets.Button):
            ...
            display(HTML("<h2>Hope this was helpful ! Thank you !<h2>"))
            pass
    ...
    def save_file(self, uploader: widgets.FileUpload):
        ...
        with warnings.catch_warnings(record=False):
            try:
                self.y_audio, self.sample_rate = \
                    librosa.load(self.uploaded_filename)

            except (RuntimeError, TypeError, audioread.NoBackendError):
                display(
                    HTML(
                        "<h2>Sorry, the file you uploaded is not "
                        "compatible, please check your music/audio"
                        " file and upload again<h2>"
                    )
                )